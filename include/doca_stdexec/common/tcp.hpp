...
public:
    ...
    // Move constructor
    tcp_socket(tcp_socket&& other) noexcept : fd_(other.fd_), connected_(other.connected_) {
        other.fd_ = -1;
    ...
    void send_dynamic(std::span<const std::byte> data, std::source_location loc = std::source_location::current()) {
