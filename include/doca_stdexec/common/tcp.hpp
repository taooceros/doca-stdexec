...
public:
  ...
  void connect(const std::string &host, std::uint16_t port) {
    ...
    if (::connect(fd_, reinterpret_cast<struct sockaddr *>(&server_addr),
      ...
      ::close(fd_);
      fd_ = -1;
      throw socket_error("Failed to connect to " + host + ":" +
                         std::to_string(port));
    }
  ...
  bool is_connected() const { return connected_ && fd_ != -1; }
  ...
  void
  send_dynamic(std::span<const std::byte> data,
               std::source_location loc = std::source_location::current()) {
    size_t size = data.size();
    std::size_t network_size = htonl(size);
  ...
  std::vector<std::byte>
  receive_dynamic(std::size_t max_size = 1024 * 1024 * 100,
                  std::source_location loc = std::source_location::current()) {
...
receive_message(tcp_socket &socket, std::size_t max_size = 1024 * 1024 * 100) {
  return socket.receive_dynamic(max_size);
}

inline std::string receive_message_string(tcp_socket &socket,
