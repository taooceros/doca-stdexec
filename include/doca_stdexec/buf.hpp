...
public:
   ...
   */
  Buf(Buf &&other) noexcept : buf_(other.buf_) { other.buf_ = nullptr; }
   ...
   */
  bool is_valid() const noexcept { return buf_ != nullptr; }

  /**
   * @brief Get the underlying doca_buf pointer
   */
  struct doca_buf *get() const noexcept { return buf_; }
   ...
   */
  template <typename T> T *get_head_as() const {
   ...
   */
  std::span<std::byte> get_data_span() const {
    void *data = get_data();
    size_t len = get_data_len();
    return std::span<std::byte>(static_cast<std::byte *>(data), len);
  }

  /**
   * @brief Get data as span of specific type
   */
  template <typename T> std::span<T> get_data_span_as() const {
   ...
   */
  template <typename T> void set_data(std::span<T> data) {
