...
struct rdma_connection_sender {
  ...
  using completion_signatures =
      stdexec::completion_signatures<stdexec::set_value_t(RdmaConnection),
                                     stdexec::set_error_t(doca_error_t),
                                     stdexec::set_stopped_t()>;

  stdexec::env<> get_env() const noexcept { return {}; }

  template <typename Receiver> struct __operation : immovable {
    ...
    static void on_established(doca_rdma_connection *connection,
                               doca_data connection_data, doca_data ctx_data) {
      auto *op = static_cast<__operation *>(connection_data.ptr);
      op->receiver.set_value(std::move(op->connection));
    }

    static void on_failure(doca_rdma_connection *connection,
                           doca_data connection_data, doca_data ctx_data) {
      auto *op = static_cast<__operation *>(connection_data.ptr);
  ...
  template <typename Receiver> auto connect(Receiver receiver) {
    return __operation<Receiver>{.connection = std::move(connection),
                                 .ctx = std::move(ctx),
                                 .receiver = std::move(receiver)};
  }

  static inline void connection_on_established(doca_rdma_connection *connection,
                                               doca_data connection_data,
                                               doca_data ctx_data) {
    printf("connection_on_established\n");
    auto *op = static_cast<__operation<uint8_t> *>(connection_data.ptr);
    op->request_cb(connection, connection_data, ctx_data);
  }

  static inline void connection_on_failure(doca_rdma_connection *connection,
                                           doca_data connection_data,
                                           doca_data ctx_data) {
...
inline void connection_request_cb(doca_rdma_connection *connection,
                                  doca_data connection_data) {
  printf("connection_request_cb\n");
}

inline void connection_disconnection_cb(doca_rdma_connection *connection,
                                        doca_data connection_data,
                                        doca_data ctx_data) {
  printf("connection_disconnection_cb\n");
}

inline static void rdma_state_changed_cb(doca_data data, doca_ctx *ctx,
                                         doca_ctx_states old_state,
...
inline Rdma::Rdma(doca_rdma *rdma) : rdma(rdma) {
  printf("rdma set connection state callbacks\n");

  auto status = doca_rdma_set_connection_state_callbacks(
      rdma, connection_request_cb,
      rdma_connection_sender::connection_on_established,
      rdma_connection_sender::connection_on_failure,
      connection_disconnection_cb);
...
inline auto Rdma::export_ctx() {
  ...
  auto ctx = std::span{reinterpret_cast<const std::byte *>(local_descriptor),
...
inline void RdmaConnection::connect(std::span<std::byte> ctx) {
  ...
  auto status =
      doca_rdma_connect(rdma->get(), ctx.data(), ctx.size(), connection.get());
...
inline rdma_connection_sender Rdma::connect(tcp::tcp_socket &socket) {
  ...
  return rdma_connection_sender{.connection = std::move(connection),
                                .ctx = std::move(received_ctx)};
}
