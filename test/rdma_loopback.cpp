...
void client() {
  ...
  auto work = schedule(context.get_scheduler()) | stdexec::then([&]() {
              ...
              stdexec::then([&](rdma::RdmaConnection connection_) {
                ...
                mmap =
                    doca_stdexec::MMap<uint8_t>(std::span<uint8_t>(local_buf));
...
void server() {
  ...
  auto work =
      schedule(context.get_scheduler()) | stdexec::then([&]() {
        context.connect_ctx(rdma);
        rdma->start();
      }) |
      stdexec::let_value([&]() {
      ...
      stdexec::let_value([&](auto &connection_) {
        ...
        dst_buf = buf_inventory->get_buffer_by_addr(
            *dst_mmap, dst_mmap->get_memrange().data(),
